...
cv_results = cv(
    ...
    fold_count=5,
    verbose=False,
    plot=True,  # Set to True if running interactively to see the learning curve
)

# Get the best number of iterations from cross-validation
best_iterations = len(cv_results["test-MultiClass-mean"])
print(f"Best number of iterations from cross-validation: {best_iterations}")

# Update parameters with the optimal number of iterations
catboost_params["iterations"] = best_iterations
